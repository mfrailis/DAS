  ...
  namespace tpl
  {
    ...
    */
	...
	return db_ == rhs.db_ && session_ == rhs.session_;
      ...
      bool operator != (const  DbBundle&rhs) const {return ! operator == (rhs);}

      bool operator == (const WeakDbBundle&rhs) const;
      bool operator != (const WeakDbBundle&rhs) const {return ! operator == (rhs);} 
	
      const std::string&
      alias() const {return db_alias_;}
	...
	return db() == rhs.db() && session() == rhs.session(); 
      ...
      const shared_ptr<odb::database>
      db() const {return db_.lock();} 
       
      const shared_ptr<odb::session>
      session() const  {return session_.lock();}
    
      bool
      expired() const {return db_.expired() || session_.expired();}
	
      bool
      blank() const {return expired() && db_alias_ == "";}
