...
void IT_OFF(IT& it, int n) {
    for (size_t i = 0; i < n; ++i) ++it;
}

template<typename X, typename Y>
void BOOST_REQUIRE_NE(const X& x, const Y& y) {
    if (x == y)
        std::cout << "error " << x << " == " << y << endl;
}

template<typename X, typename Y>
void BOOST_CHECK_EQUAL(const X& x, const Y& y) {
    if (x != y)
        std::cout << "error " << x << " != " << y << endl;
}
...
void image_2D_test_blob(shared_ptr<D::Database> db, const std::string path = "") {
    ...
    ptr = test_image2d_blob::create("image2d_unit_test", "test_level2");
    long long id;
    ...
    ptr->append_tiles(tiles_1);

    ptr->append_tiles(tile_1);
    ...
    t = db->begin();
    ptr = db->load<test_image2d_blob>(id);
    t.commit();
...
void image_3D_test_blob(shared_ptr<D::Database> db, const std::string path = "") {
    ...
    ptr = test_image3d_blob::create("image3d_unit_test", "test_level2");
    long long id;
    ...
    t = db->begin();
    ptr = db->load<test_image3d_blob>(id);
    t.commit();
